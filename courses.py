...
def fetch_details(course_id):
    sql = "SELECT id, name, credits, description FROM courses WHERE courses.id=:course_id"
    result = db.session.execute(text(sql), {"course_id":course_id}).fetchone()
    return result

def fetch_comments(course_id):
    sql = "SELECT users.username, comments.content, comments.post_date FROM comments JOIN users ON comments.author_id = users.id WHERE comments.course_id=:course_id"
