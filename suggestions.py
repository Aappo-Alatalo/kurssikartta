...
def create_suggestion(title, op, description, user_id):
    try:
        ...
        db.session.execute(
            text(sql),
            {"title": title, "op": op, "description": description, "user_id": user_id},
        )
        db.session.commit()
        return True
    except:
...
def accept_suggestion(suggestion_id):
    try:
        ...
        sql_fetch = (
            "SELECT name, credits, description FROM suggestions WHERE id=:suggestion_id"
        )
        ...
        if suggestion:
            ...
            sql_update = (
                "UPDATE suggestions SET status='accepted' WHERE id=:suggestion_id"
            )
